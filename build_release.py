...
with alive_bar(len(combos)) as bar:
    for combo in combos:
        try:
            ...
            # Variables
            cc = cross_compilers[combo]
            goos = combo.split('/')[0]
            goarch = combo.split('/')[1]
            args = ""
            if goos == "windows":
                args += "-ldflags '-H windowsgui'"
            # Render command(s)
            build_command = (
                f"CC={cc} GOOS={goos} GOARCH={goarch} CGO_ENABLED=1 "
                f"go build {args} -o {combo_path}/core"
            )
            # Create a subprocess for the command to be executed
            subprocess.run(build_command, shell=True, check=True)
            # Generate SHA256 checksum
            sha512 = hashlib.sha512()
            with open(f"{combo_path}/core", 'rb') as f:
                for chunk in iter(lambda: f.read(4096), b""):
                    sha512.update(chunk)
            sha512_checksum = sha512.hexdigest()
            with open(f"{combo_path}/core.sha512", 'w') as sha_file:
                sha_file.write(f"{sha512_checksum}  {combo_path}/core")
            bar()
...
for folder in Path(f"{bin_directory}/windows").glob("*"):
    core_file = folder / "core"
    if core_file.exists():
        core_file.rename(core_file.with_suffix(".exe"))
